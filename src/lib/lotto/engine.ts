// @ts-nocheck
/* Port of lottery-dashboard engine.js — analysis, prediction, prize verification. */
const WHITE_PICK = 5;
...
export const GAME_CONFIGS = {
  mega: {
    ...
    megaMax: 24,
    ballLabel: 'Mega Ball',
    historyUrl: '/history/mega_history.json', // bundled full archive (2002-present); live NY open-data merged on top
  ...
  powerball: {
    ...
    megaMax: 26,
    ballLabel: 'Powerball',
    historyUrl: '/history/powerball_history.json', // bundled archive (2010-present); live NY open-data merged on top
  ...
  hit5: {
    ...
    hasBonus: false,
    ballLabel: 'Bonus',
    historyUrl: '/history/hit5_history.json', // bundled with the app
  },
  walotto: {
    id: 'walotto',
    ...
    hasBonus: false,
    ballLabel: 'Bonus',
    historyUrl: '/history/walotto_history.json', // bundled with the app
  }
};
...
function normalizeHistoryRow(row, config) {
  ...
  const drawDate = lower.draw_date || lower.date || lower.drawdate || lower.drawn_at || lower.draw || lower.drawdate;
  let numbers = [];
  let megaBall = NaN;
  ...
  if (Array.isArray(lower.bonus_numbers) && lower.bonus_numbers.length) {
    megaBall = Number(lower.bonus_numbers[0]);
  } else if (lower.megaball != null && lower.megaball !== '') {
    megaBall = Number(lower.megaball);
  }
...
export const TICKET_PRICES = { mega: 5, powerball: 2, walotto: 1, hit5: 1 };
